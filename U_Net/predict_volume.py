...
if __name__ == "__main__":
    ...
    trans_XZ = np.moveaxis(XZ_volume, 0, 1)
    trans_YZ = np.moveaxis(YZ_volume, 0, -1)
    
    print("Predictions shape XY:", XY_volume.shape)
    print("Predictions shape XZ_trans:", trans_XZ.shape)
    print("Predictions shape YZ_trans:", trans_YZ.shape)
    
    predictions = np.zeros((3, XY_volume.shape[0], XY_volume.shape[1], XY_volume.shape[2]))
    predictions[0] = XY_volume
    predictions[1] = trans_XZ
    predictions[2] = trans_YZ
    
    print("Predictions shape:", predictions.shape)
    print("Values:", np.unique(XY_volume), np.unique(trans_XZ), np.unique(trans_YZ))
    
    import matplotlib.pyplot as plt
    
    plt.figure(0)
    plt.imshow(predictions[0,500,:,:])
    plt.figure(1)
    plt.imshow(predictions[1,500,:,:])
    plt.figure(2)
    plt.imshow(predictions[2,500,:,:])
    plt.show()
